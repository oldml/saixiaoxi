<template>
  <div class="space-y-5">
    ...
    >
      <Section v-if="isVisible(0)" title="装备与外观">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
          <InfoCard label="头部" :value="formatClothes(userinfo.clothes?.[0])" />
          <InfoCard label="手部" :value="formatClothes(userinfo.clothes?.[1])" />
          <InfoCard label="腰部" :value="formatClothes(userinfo.clothes?.[2])" />
          <InfoCard label="腿部" :value="formatClothes(userinfo.clothes?.[3])" />
          <InfoCard label="目镜" :value="formatClothes(userinfo.clothes?.[4])" />
        </div>
      </Section>
    </Transition>
    ...
    >
      <div
        v-if="isVisible(7)"
        class="rounded-xl border border-white/12 bg-white/6 p-5 text-slate-100 shadow-[0_12px_34px_rgba(2,6,23,0.3)] backdrop-blur-lg"
      >
        <button
          @click="debugVisible = !debugVisible"
          class="flex w-full items-center justify-between text-sm font-medium text-slate-200/85 transition-colors hover:text-sky-300"
        >
          <span>调试信息</span>
          <ChevronDown :class="['h-4 w-4 transition-transform', debugVisible && 'rotate-180']" />
        </button>
        <div
          v-show="debugVisible"
          class="mt-3 max-h-80 overflow-auto rounded-lg border border-white/10 bg-white/8 p-3 text-left text-xs text-slate-200/70 backdrop-blur"
        >
